# TapTap Godot 3.x 插件工具说明

## 📦 插件内容

### 核心文件
- **`plugin.cfg`** - 插件配置文件
- **`plugin.gd`** - 插件初始化脚本
- **`taptap.gd`** - TapTap SDK 主要接口类

### 加密相关工具
- **`TapTapConfigWindow.gd`** - Token 加密配置工具（GUI）
- **`generate_secure_key.gd`** - 密钥生成工具（命令行）

### 文档
- **`CLOUDSAVE_USAGE.md`** - 云存档功能详细使用指南

### 示例文件
- **`example_usage.gd`** - 使用示例代码

## ✨ 功能特性

- 🔐 **TapTap 登录** - 支持 TapTap 账号登录
- ✅ **正版验证** - 游戏版权验证
- 🎮 **DLC 管理** - DLC 商品查询和购买
- 💰 **内购系统** - 完整的 IAP 支付流程
- 📜 **合规认证** - 防沉迷系统集成
- ☁️ **云存档** - 跨设备存档同步（新增）

## 🔧 工具使用方法

### 1. Token 加密配置工具（推荐）

这是一个图形界面工具，提供完整的密钥管理和 Token 加密功能。

**启动方式**：
```
Project → Tools → TapTap RSA 密钥配置
```

**功能特性**：
- 🔑 密钥管理：读取/保存/生成密钥
- 🔐 Token 加密：输入明文 Token，输出加密结果
- 📋 代码生成：自动生成 Java 解密代码
- 💾 文件同步：自动与 Android 资源文件同步

**使用流程**：
1. 打开配置工具
2. 确认或生成密钥
3. 输入 Client Token
4. 点击加密生成结果
5. 复制加密 Token 和 Java 代码

### 2. 密钥生成工具（命令行）

这是一个 EditorScript 工具，用于快速生成和替换密钥。

**启动方式**：
1. 在 Godot 编辑器中打开 `addons/godot3-taptap/generate_secure_key.gd`
2. 按 **Ctrl+Shift+X** 或点击 **File → Run**

**功能特性**：
- 🎲 生成安全的随机密钥
- 💾 自动保存到 Android 资源文件
- 📝 提供详细的操作指引

**使用场景**：
- 批量密钥更新
- 自动化脚本集成
- 快速密钥轮换

## 🔒 安全配置

### 密钥文件位置
```
android/build/res/raw/taptap_decrypt_key.txt
```

### 密钥格式
- **格式**：纯文本，单行密钥
- **长度**：建议 16-32 个字符
- **字符集**：字母数字组合，避免特殊字符

### 版本控制
建议将密钥文件添加到 `.gitignore`：
```gitignore
# TapTap 密钥文件
android/build/res/raw/taptap_decrypt_key.txt
```

## 🚀 快速开始

### 步骤 1：配置密钥
```
Project → Tools → TapTap RSA 密钥配置
点击 "🎲 生成随机密钥" → "💾 保存密钥"
```

### 步骤 2：加密 Token
```
在配置工具中输入 Client Token
点击 "🔐 生成加密 Token"
复制加密结果
```

### 步骤 3：更新代码
```gdscript
# 在游戏中使用加密 Token
TapTap.initSdkWithEncryptedToken("加密后的Token", clientId, false)
```

### 步骤 4：更新 Java 代码
将配置工具生成的 Java 代码添加到 `Godot3TapTap.java` 中。

## 📋 API 参考

### TapTap 主要方法

```gdscript
# 普通初始化（明文 Token）
TapTap.initSdk(clientId: String, clientToken: String, enableLog: bool)

# 加密初始化（推荐）
TapTap.initSdkWithEncryptedToken(encryptedToken: String, clientId: String, enableLog: bool)

# 登录
TapTap.startTapLogin(permissions: Array)

# 获取用户信息
TapTap.getCurrentUser()

# 登出
TapTap.logout()
```

### 加密工具方法

```gdscript
# TapTapConfigWindow.gd 主要方法
_load_or_create_key()          # 加载或创建密钥
_save_key_to_file(key: String) # 保存密钥到文件
_simple_encrypt(text: String)   # XOR 加密
_generate_java_decrypt_code()   # 生成 Java 代码

# generate_secure_key.gd 主要功能
_run()  # 生成随机密钥并保存
```

## 🔍 故障排除

### 常见问题

**Q: 配置工具无法保存密钥**
A: 检查 `android/build/res/raw/` 目录是否存在，手动创建该目录。

**Q: 加密 Token 在 Android 端解密失败**
A: 确认密钥文件内容与配置工具中的密钥一致。

**Q: 找不到配置工具菜单**
A: 确保插件已启用，重启 Godot 编辑器。

### 调试方法

1. **检查密钥文件**：
   ```
   cat android/build/res/raw/taptap_decrypt_key.txt
   ```

2. **验证加密功能**：
   使用配置工具加密测试 Token，检查输出是否正常。

3. **查看日志**：
   在 Android 端查看解密相关的错误日志。

## 📁 项目结构

```
godot3_taptap/
├── addons/godot3-taptap/              # 插件文件夹
│   ├── plugin.cfg                     # 插件配置
│   ├── plugin.gd                      # 插件入口
│   ├── taptap.gd                      # 主接口
│   ├── TapTapConfigWindow.gd          # 配置工具
│   ├── generate_secure_key.gd         # 密钥生成器
│   ├── CLOUDSAVE_USAGE.md             # 云存档使用指南
│   └── example_usage.gd               # 使用示例
├── android/build/res/raw/
│   └── taptap_decrypt_key.txt         # 密钥文件
├── Scenes/
│   └── Main.gd                        # 游戏主场景
└── 📄 文档文件...
```

## ☁️ 云存档功能

TapTap 云存档功能已集成到本插件中，支持跨设备存档同步。

### 快速使用

```gdscript
# 1. 初始化 SDK（确保已登录）
TapTap.initSdkWithEncryptedToken("client_id", "encrypted_token", true)
TapTap.login(true, false)

# 2. 创建存档
TapTap.createArchive(
    "save_slot_1",           # 存档名称（仅英文/数字/下划线/中划线）
    "关卡 10 - 1000分",       # 存档摘要（可使用中文）
    "{}",                     # 额外信息
    3600,                     # 游戏时长（秒）
    "user://save.json",       # 存档文件路径
    "user://cover.png"        # 封面路径（可选）
)

# 3. 获取存档列表
TapTap.getArchiveList()

# 4. 下载存档
TapTap.getArchiveData(archive_uuid, file_id)

# 5. 更新存档
TapTap.updateArchive(archive_uuid, name, summary, extra, playtime, file_path, cover_path)

# 6. 删除存档
TapTap.deleteArchive(archive_uuid)
```

### 连接信号

```gdscript
TapTap.connect("onCreateArchiveSuccess", self, "_on_create_success")
TapTap.connect("onGetArchiveListSuccess", self, "_on_list_loaded")
TapTap.connect("onGetArchiveDataSuccess", self, "_on_data_downloaded")
```

### 详细文档

查看 [CLOUDSAVE_USAGE.md](CLOUDSAVE_USAGE.md) 获取完整的云存档使用指南，包括：
- 完整 API 说明
- 错误处理指南
- 最佳实践建议
- 完整游戏流程示例

## 📄 相关文档

- **`CLOUDSAVE_USAGE.md`** - 云存档功能详细使用指南（新增）
- **`TapTap_Token加密配置指南_v2.md`** - 详细的使用指南
- **`android/Android资源配置指南.md`** - Android 配置说明

## 🔄 版本历史

### v2.0 (当前版本)
- ✅ 可配置密钥系统
- ✅ 图形界面配置工具
- ✅ 自动文件管理
- ✅ 简化的工作流程

### v1.0
- ✅ 基础 XOR 加密
- ✅ 固定密钥系统
- ✅ 手动配置流程